...
public class TraktAuthenticationViewController: UIViewController {
	...
	var responseCode: GeneratedCodeResponse? {
		didSet {
			...
			if responseCode != nil {
				...
				intervalTimer = Timer.scheduledTimer(timeInterval: responseCode!.interval, target: self, selector: #selector(poll(timer:)), userInfo: nil, repeats: true)
			}
		}
	}
	...
	private func getNewCode() {
		...
		TraktRequestGenerateCode(clientId: trakt.clientId).request(trakt: trakt) { [weak self] data, error in
	...
	@objc public func poll(timer: Timer) {
		if responseCode?.expiresAt.compare(NSDate() as Date) == .orderedAscending || responseCode == nil {
			timer.invalidate()
			getNewCode()
		} else {
			activity.startAnimating()

			TraktRequestPollDevice(trakt: trakt, deviceCode: responseCode!.deviceCode).request(trakt: trakt) { [weak self] token, error in
                self?.activity.stopAnimating()
                if token != nil {
                    timer.invalidate()
